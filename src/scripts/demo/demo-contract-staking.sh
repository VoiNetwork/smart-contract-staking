...
main() {
  ...
  local period
  local word
  config_type=staking-factory
  config_name=staking-demo
  config_seconds=1
  config_lockup_delay=1
  config_distribution_count=1
  config_period_limit=17
...
EOF
  ...
  for period in $( seq 1 ${config_period_limit} )
  do
    echo "period:${period}"
    app_id=$( cli factory deploy-staking --apid ${fapid} --amount 1 --period ${period} )
    test ! ${app_id} -eq 0 || {
